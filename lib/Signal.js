...
Signal.SignalHandler.prototype.registerSignalHandler = function(type, handler)
{
  ...
  if (this[name])
    throw 'Cannot have more than one handler for signal on a single object';
...
{
  this.sendSignal({ showShit: null });
}

/**
 * Fire an event that will request a signal be sent
 * @param {String} type Message type
 * @param {*=} parameter Optional additional information
 */
Signal.SignalSender.prototype.sendSignal = function(type, paramter)
{
  this.trigger(Signal.SEND_SIGNAL, new Signal(this, type, paramter));
};
