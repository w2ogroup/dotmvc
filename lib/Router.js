...
{
  var matches = getName(TController).match(/^(.*)Controller$/);
  if (!matches)
    throw getName(TController) + ' is not a valid controller class name';
...
Router.prototype.addRoute = function(route, TController, method)
{
  ...
  var sig = getName(TController) + '::' + method;
...
Router.prototype._executeRoute = function(TController, method, route)
{
  ...
  if (!this._controller) {
    ...
    app.window.element.className = getName(TController)
