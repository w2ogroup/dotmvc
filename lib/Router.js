...
// A super-powered Backbone router that lets use use controllers as the primary
// means of establishing routes and behavior
...
// Setup all the routes for a controller using lots of MAGIC to build routes
// just from the class itself
...
// Helper function to add on the optional route matches based on a functions
// parameters
...
// potential controller swapping / caching / state management semantics would
// occur, IF WE HAD ANY
Router.prototype._executeRoute = function(TController, method, route)
{
  ...
  if (this._controller && this._controller.constructor !== TController) {
    console.log(this._controller + ' unloading...');
