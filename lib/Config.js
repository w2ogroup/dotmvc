module.exports = Config;

/**
 * @constructor
 */
function Config()
{
  this._config = {};
}

var NOT_FOUND;

/**
 * @return {bool} True if the config store has a key.
 * @param {String} key
 */
Config.prototype.has = function(key)
{
  return this.get(key) !== NOT_FOUND;
};

/**
 * Get a value from the config.
 * @param {String} key
 */
Config.prototype.get = function(key)
{
  if (!key) return;

  var parts = key.split('.');
  var root = this._config;

  for (var n = 0; n < parts.length - 1; n++) {
    var p = parts[n];
    if (root[p] === undefined) return NOT_FOUND;
    root = root[p];
  }

  return root[parts[parts.length-1]];
};

/**
 * Add in an object to our current config
 * @param {Object} options Hash of configs.
 */
Config.prototype.merge = function(options)
{
  deepExtend(this._config, options);
};

function deepExtend(target, source)
{
  for (var prop in source) {
    if (prop in target && typeof target[prop] === 'object') {
      deepExtend(target[prop], source[prop]);
    } else {
      target[prop] = source[prop];
    }
  }
}

/**
 * @param {String} key
 * @param {*} value
 */
Config.prototype.set = function(key, value)
{
  if (!key) return;
  if (typeof key === 'object') return this.merge(key);

  var parts = key.split('.');
  var root = this._config;

  // Move root pointer to second-to-last part of the dot expression, creating
  // nodes along the way if we don't have them
  for (var n = 0; n < parts.length - 1; n++) {
    var p = parts[n];
    if (root[p] !== undefined) {
      root = root[p];
      continue;
    }
    var _ref = {};
    root[p] = _ref;
    root = _ref;
  }

  root[parts[parts.length - 1]] = value;
};



