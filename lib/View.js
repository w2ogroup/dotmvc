...
// Fired off to propigate messages from context's send message
...
View.prototype.executeCommand = function(hash)
{
  ...
  for (var command in hash) {
    ...
    this.$element.trigger(View._REQUEST_COMMAND_HANDLER, [command, opt, this]);
  }
...
View.prototype.setDataContext = function(context)
{
  ...
  if (context && context.on instanceof Function) {
    ...
    this.listenTo(context, Messanger.MESSAGE, function(blast) {
      this.$element.trigger(
        View._NOTIFY_MESSAGE_SENT,
        [blast.message, blast.param, blast.from, this]);
    });
  }
...
View.prototype.clear = function()
{
  if (this._isHtmlSacred())
    log.warn('Internal DOM identity shouldn\'t change! Views could be lost');
...
// further bubbling
View.prototype._onCommandRequested = function(e, command, opt, from)
{
  if (!this._context) return;

  var handled = CommandProvider.requestCommand(this._context, command, opt);

  if (handled) {
    log(this._context + ' <- ' + command + ' - ' + this + ' <- ' + from);
    e.stopPropagation();
  }
};

// Can we do this message?
View.prototype._onMessageNotify = function(e, message, param, from, start)
{
  log(from + ' - ' + message + ' -> ' + start + ' -> ' + this);
  if (!this._messages[message]) return;

  this._messages[message](param);
  e.stopPropagation();
};
