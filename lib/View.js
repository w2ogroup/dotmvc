...
var debug      = require('debug')('dotmvc:View');
...
function View(domElement, id)
{
  ...
  if (domElement && T.DOM_NODE &&
    ...
    debug('View node name mismatch: Expected ' +
...
View.prototype.addView = function(view)
{
  if (this._couldHtmlGetClobbered())
    debug('DOM identity not safe! addView() is dangerous');
...
{
  if (this._couldHtmlGetClobbered())
    debug('DOM identity not safe! createViews() is dangerous');
...
View.prototype.setDataContext = function(context)
{
  ...
  debug(this + ' data context changed to -> ' + context);
...
View.prototype._onContextChange = function()
{
  ...
  // If there's already a pending change, bounce
  if (this._contextChangeId) {
    return;
  }

  // This will set a timeout for 1 30fps frame. The idea is to limit actualy
  // change propigations to only once every frame
  var _this = this;
...
View.prototype._onCommandRequested = function(e, command, opt)
{
  ...
  if (this._context[command] instanceof Function) {
    debug(this._context + ' <- ' + command + ' - ' + this);
