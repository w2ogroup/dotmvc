...
// Templating function -- if we have one, the DOM will be updated (re-written!)
// every render
View.TEMPLATE  = null;

// Layout function -- if we have one, this will setup the DOM only ONCE in its
// lifetime. Used to skeleton out subviews
View.LAYOUT    = null;
...
View.prototype.clearHtml = function()
{
  if (this._couldHtmlGetClobbered())
    log.warn('DOM identity not safe! clearHtml() is dangerous');
