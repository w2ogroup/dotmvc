...
View.prototype.executeCommand = function(hash)
{
  ...
  for (var command in hash) {
    var param = hash[command];
    this.$element.trigger(View._REQUEST_COMMAND_HANDLER, [command, param, this]);
  }
...
// Manually trigger a message from the view itself
View.prototype.sendMessage = function(hash)
{
  for (var name in hash) {
    var param = hash[name];
    var m = new Messanger.Message(this, name, param);
    this.$element.trigger(View._NOTIFY_MESSAGE_SENT, [m, this]);
  }
};
...
// further bubbling
View.prototype._onCommandRequested = function(e, command, opt, start)
{
