...
var Binding    = require('smack').Binding;
...
module.exports = View;
mixin_(View, Observable);
mixin_(View, Signal.SignalHandler);
mixin_(View, stringy(View));
 ...
 * var extends_ = require('dotmvc/lib/util/extends');
 *
 * module.exports = HelloView;
 * extends_(HelloView, View);
 *
 * function HelloView()
 * {
...
function View(domElement)
{
  ...
  if (domElement && T.DOM_NODE &&
    ...
    throw new Error ('View node name mismatch: Expected ' +
...
{
  if (this._started)
    throw new Error('Cannot init a view more than once');
...
View.prototype.render = function(context)
{
  ...
  if (!this.element)
    throw new Error('Attempting to render ' +
...
View.prototype.addView = function(view)
{
  if (this._couldHtmlGetClobbered())
    throw new Error('DOM identity not safe! addView() is dangerous');
...
function renderView(view)
{
  view.render();
  if (!view._rendered)
    throw new Error('View not rendered properly, ' +
