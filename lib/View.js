...
module.exports = (function() {
  ...
  {
    var element = domElement || document.createElement('div');
    var T = this.constructor;

    this.id         = id || _.uniqueId();
    this.element    = element;
    this.$element   = $(element);
    this.template   = this.template || T.TEMPLATE || null;
    ...
    // Set the DOM element id and class name
    this.element.id        = 'view-' + this.id;
    this.element.className =
      'view' + (T.CSS_CLASS ?  ' ' + T.CSS_CLASS : '');
  ...
  View.REQUEST_COMMAND_HANDLER = 'View::REQUEST_COMMAND_HANDLER';
  ...
  View.prototype.render = function()
  {
    ...
    if (this._rendered && this.renderOnce) return;

    // If this view is template driven, dump the HTML every time we request a
    // render. This may not be what we want.
    if (this.template)
      this._injectTemplate();
  ...
  View.prototype.executeCommand = function(hash)
  {
    ...
    for (var command in hash) {
      ...
      this.$element.trigger(View.REQUEST_COMMAND_HANDLER, [command, opt]);
    }
    return;
  };
  ...
  View.prototype.setDataContext = function(context)
  {
    ...
    if (context && context.on instanceof Function)
      this.listenTo(context, Observable.CHANGE, this.render);
    this.render();
  ...
  // Should be set to true if we dont want to clobber innerHtml whenever a
  // datacontext is updated
  View.prototype.setRenderOnce = function(renderOnce)
  {
  ...
  // Set the html to our template (semi-agnostic so long as the API is the same)
  View.prototype._injectTemplate = function()
  {
